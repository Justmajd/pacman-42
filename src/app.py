...
from src.contracts import Direction, GameSnapshot, GameState, WorldEvent, GhostState
...
def run_app(config: GameConfig) -> int:
    ...
    for ghost_id,spawn in enumerate(level_data.ghost_spawns):
        ghost: Ghost = Ghost(position=spawn ,spawn=spawn , direction=Direction.NONE,active=True,respawn_delay=0.0,ghost_id=ghost_id)
        ghosts.append(ghost)
    ...
    while renderer.is_running:
        ...
        if session.state == GameState.PLAYING:
            if level_start_countdown > 0:
                ...
                for ghost in ghosts:
                    ghost.update(dt=dt, grid=grid,target=player.position,rng=rng,frightened=(session.frightened_time_remaining>0))
                ...
                for event in consumable:
                    session.handle_event(event=event)
                    if event == WorldEvent.LEVEL_CLEARED:
                        if session.state == GameState.PLAYING:
                            level_data = stubmaze.build_level(session.level, config.seed)
                            world = World(level=level_data)
                            grid = Grid(level=level_data)
                            player = Player(position=level_data.player_spawn,direction=Direction.NONE,spawn=level_data.player_spawn,requested_direction=Direction.NONE, lives=session.lives)
                            ghosts = []
                            for ghost_index,spawn in enumerate(level_data.ghost_spawns):
                                ghost = Ghost(position=spawn ,spawn=spawn , direction=Direction.NONE,active=True,respawn_delay=0.0,ghost_id=ghost_index)
                                ghosts.append(ghost)
                            renderer.load_level(level_data=level_data)
                            transition = True
                            level_start_countdown = 3.0
                if transition is False and session.state == GameState.PLAYING:
        ...
        for ghost in ghosts:
            ghost_state = GhostState(id=ghost.ghost_id,position=ghost.render_position(),direction=ghost.direction,is_frightened=(session.frightened_time_remaining>0),frightened_time_remaining=session.frightened_time_remaining,is_active=ghost.active,is_eaten=ghost.is_eaten)
            ghosts_state.append(ghost_state)
        ghost_state_tuple = tuple(ghosts_state)
        game_snapshot = GameSnapshot(level_start_countdown=level_start_countdown,player_pos=player.render_position(),player_direction=player.facing,player_is_dying=(session.state == GameState.GAME_OVER),player_is_moving=(player.is_moving()),pacgums=world.pacgums,super_pacgums=world.super_pacgums,ghosts=ghost_state_tuple,score=session.score,level=session.level,lives=session.lives,time=session.level_time_remaining)
